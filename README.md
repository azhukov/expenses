...
time — waits for every healthcheck, then runs and opens the browser client. Run with `NO_FE=1` to
skip it for the containers alone, or `FE_HOST=1` to expose the browser client on the LAN over
HTTPS, at `https://<machine-name>.local:5173`.
...
`SKIP_BUILD`, `NO_FE`, `FE_HOST`). On Windows, run them from a Git Bash terminal.
