...
- [ ] 1.1 Move `examples/1000023157.jpg`, `1000023218.jpg`, `1000023219.jpg` into the test fixture
      directory and commit them. They are currently untracked.
- [ ] 1.2 Record the verify response for `1000023219` and commit it beside the images as
      `verify-32AA324CFF5030271E16D59F7F8EF636.json`. This is the only sanctioned copy; tests never
      call the portal (D26).
- [ ] 1.3 Add the zxing-cpp package reference and remove `ZXing.Net` from
      `Expenses.Infrastructure.csproj` (D21).
- [ ] 1.4 Confirm the native binaries zxing-cpp ships resolve in the container image, not only on
      the dev machine.

## 2. Decoding

- [ ] 2.0 Write the failing decoder tests. `1000023219.jpg` decodes from the unmodified full-resolution
...
- [ ] 2.1 Replace the decoder implementation behind `IFiscalCodeDecoder` with zxing-cpp.
- [ ] 2.2 Delete the scale ladder and the time budget. The measured hit needed neither, and neither
      rescued a measured miss (D21).

## 3. Reading the decoded payload

- [ ] 3.0 Write the failing payload tests against the decoded string for `1000023219`: the fragment
...
- [ ] 3.1 Correct `FiscalIdentity.From` so JIKR is never populated from `crtd` (D24).
- [ ] 3.2 Carry the total, the issuer tax number and the creation timestamp out of the payload
      alongside the identifiers.
- [ ] 3.3 Make fiscal identifier comparison insensitive to hyphenation, since the JIKR is printed
      hyphenated by one ERP and unhyphenated by another (D24).

## 4. Retrieving the invoice

- [ ] 4.0 Write the failing portal-client tests against the committed recording: the response maps to
      ...
      reached].
- [ ] 4.1 Add the port for retrieving an invoice by decoded identity, alongside `IFiscalCodeDecoder`.
- [ ] 4.2 Implement the adapter: form-encoded `iic`, `dateTimeCreated`, `tin`; bounded timeout;
      every failure returns nothing rather than throwing (D26).
- [ ] 4.3 Map the response to an `ExtractionResult`, taking values verbatim (D27).
- [ ] 4.4 Reuse an already-retrieved invoice when extraction is re-run [Retrieval is not repeated
      needlessly].

## 5. Cascade ordering

- [ ] 5.0 Write the failing cascade tests: a decoded-and-retrieved invoice that validates leaves the
...
- [ ] 5.1 Add the retrieval stage as `Primary` and demote the placeholder vision stages to
      `Fallback` (D22).
- [ ] 5.2 Register the stages in the new order and add the portal options to configuration.

## 6. Validation precision

- [ ] 6.0 Write the failing validation test: lines summing raw to `59.6515` against a stated total of
...
- [ ] 6.1 Round each side to two decimal places before comparison, in the sum, discount and tax
      checks alike (D25).

## 7. End to end

- [ ] 7.0 Write the failing journey test: uploading `1000023219.jpg` to a purchase yields candidates
...
- [ ] 7.1 Make it pass.
- [ ] 7.2 Update the extraction MCP tool and HTTP responses to surface the retrieval stage as the
...
- [ ] 8.1 Record the measured decoder comparison and the observed portal contract in the repository,
      so the next person does not repeat the spike.
- [ ] 8.2 Note the open question this change does not answer: whether the portal still serves an old
