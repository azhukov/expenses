...
public interface IExtractionCandidateStore
{
    ...
    Task<ExtractionResult?> FindLatest(long purchaseId, CancellationToken cancellationToken = default);

    /// <summary>Replaces any unconfirmed candidates; re-running is non-destructive to expenses.</summary>
    Task Replace(long purchaseId, ExtractionResult result, CancellationToken cancellationToken = default);
