    ...
    protected override void Up(MigrationBuilder migrationBuilder)
    {
        ...
        migrationBuilder.CreateTable(
            ...
            columns: table => new
            {
                ...
                receipt_content_hash = table.Column<byte[]>(type: "bytea", nullable: true),
                receipt_storage_key = table.Column<string>(type: "text", nullable: true),
                receipt_content_type = table.Column<string>(type: "varchar(128)", nullable: true),
                receipt_size_in_bytes = table.Column<long>(type: "bigint", nullable: true),
                receipt_state = table.Column<int>(type: "integer", nullable: true),
                receipt_failure_reason = table.Column<string>(type: "text", nullable: true),
                ...
                fiscal_extracted_source = table.Column<int>(type: "integer", nullable: true)
            },
            constraints: table =>
            {
                table.PrimaryKey("PK_purchases", x => x.id);
                table.CheckConstraint("ck_purchases_merchant_raw_length", "merchant_raw IS NULL OR length(merchant_raw) <= 512");
                table.CheckConstraint("ck_purchases_receipt_all_or_nothing", "(receipt_content_hash IS NULL AND receipt_storage_key IS NULL AND receipt_content_type IS NULL\n    AND receipt_size_in_bytes IS NULL AND receipt_state IS NULL)\nOR (receipt_content_hash IS NOT NULL AND receipt_storage_key IS NOT NULL\n    AND receipt_content_type IS NOT NULL AND receipt_size_in_bytes IS NOT NULL\n    AND receipt_state IS NOT NULL)");
                table.CheckConstraint("ck_purchases_receipt_content_hash_length", "receipt_content_hash IS NULL OR length(receipt_content_hash) = 32");
                table.CheckConstraint("ck_purchases_receipt_storage_key_length", "receipt_storage_key IS NULL OR length(receipt_storage_key) <= 256");
                table.ForeignKey(
        ...
        migrationBuilder.InsertData(
            table: "units",
            columns: ["id", "code", "is_active", "kind", "name", "symbol"],
            values: new object[,]
            {
